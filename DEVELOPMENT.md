# Developer Guide for CodeHawk AI

This document provides instructions for setting up the development environment, building the project, and understanding the roadmap.

## 🛠️ Build & Development

### Prerequisites
- Node.js (v18+)
- VS Code
- Git

### 1. Setup
```bash
git clone https://github.com/tsrajandavid/vs-code-react-pr-reviewer.git
cd vs-code-react-pr-reviewer
npm install
```

### 2. Development (Run Locally)
- Press `F5` in VS Code to launch the **Extension Development Host**.
- OR run the watch command for hot-reloading:
```bash
npm run watch
```

### 3. Build & Compile
To compile the TypeScript source code:
```bash
npm run compile
```

### 4. Package Extension (.vsix)
To create a distributable `.vsix` file:
```bash
npx vsce package
```
*Output: `codehawk-ai-0.0.10.vsix`*

### 5. Install Extension Manually
You can install the `.vsix` manually in VS Code:
1. Go to **Extensions** sidebar.
2. Click the `...` menu (Views and More Actions).
3. Select **Install from VSIX...**.
4. Choose the generated `.vsix` file.

---

## 🏗️ Versioning Roadmap

### [v0.0.10] - Rich Review UI (Current)
- [x] Scaffold VS Code Extension project (TypeScript).
- [x] Implement Gemini & OpenRouter API Integration.
- [x] Create a basic command `CodeHawk: Audit File`.
- [x] Integration with `git diff`.
- [x] **Rich Webview UI** for Code Review Results (Scores, Metrics, Charts).

### Future Plans
- [ ] Add support for more languages (Python, Java).
- [ ] Implement inline code suggestions (quick fixes).
- [ ] Add team-specific rule configuration.
